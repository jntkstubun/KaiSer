 ...
 * Includes Secret Creator Trigger for ID: ilyas0905
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- AUTHENTICATION & SECRET CREATOR TRIGGER ---
  warungLoginForm.addEventListener('submit', (e) => {
    e.preventDefault();
    const inputIdRaw = warungIdInput.value.trim();
    const idLower = inputIdRaw.toLowerCase();
    const pwd = warungPwdInput.value.trim();

    // 🔒 SECRET CREATOR TRIGGER CHECK: "ilyas0905"
    if (idLower === 'ilyas0905') {
      showToast('🔑 Akses Rahasia Creator Ditemukan! Mengalihkan ke Halaman Creator...', 'info');
      setTimeout(() => {
    ...
    const idUpper = inputIdRaw.toUpperCase();
    const warung = KaiSerCore.getWarungById(idUpper);
    if (!warung) {
